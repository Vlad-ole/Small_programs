...
//New GEM:
#define _X_SIZE 0.0275
#define _Y_SIZE 0.047631397208
#define MESH_ std::string("../v00.01_THGEM0/")
#define RESULT_ std::string("case_v01.result")
...
int main(int argc, char * argv[]) {
  ...
  bool is_loaded = fm->Initialise(MESH_+"mesh.header", MESH_+"mesh.elements",MESH_+ "mesh.nodes",MESH_+ "diels.dat",MESH_+RESULT_,"mm");
  if (is_loaded)
  ...
  for (int ff=0;ff<fm->GetNumberOfMaterials();ff++)
     fm->DriftMedium(ff);
    // Make a medium
  LAr_Mat* LAr = new LAr_Mat("LAr_drift_data.txt", "LAr_diffusionL_data.txt","LAr_diffusionT_data.txt");
  ...
  sensor->SetArea(-5.5*_X_SIZE,-5.5*_Y_SIZE,-0.15, 5.5*_X_SIZE, 5.5*_Y_SIZE, 0.15);
  TCanvas* c3 = new TCanvas("drift lines xz", "drift lines xz");

  //plot_field(fm,"test_E.txt",_X_SIZE,_Y_SIZE,-0.31,_X_SIZE,_Y_SIZE,0.31, 2000,"axis z",0.06,1500);
  //plot_field(fm,"test_E2_1.txt",(_X_SIZE-0.05),_Y_SIZE,0,(_X_SIZE+0.045),_Y_SIZE,0, 2000);
  //plot_field(fm,"test_E2_2.txt",(_X_SIZE-0.05),_Y_SIZE,0.01,(_X_SIZE+0.045),_Y_SIZE,0.01, 2000);
  //plot_field(fm,"test_E2_3.txt",(_X_SIZE-0.05),_Y_SIZE,_X_SIZE,(_X_SIZE+0.045),_Y_SIZE,_X_SIZE, 2000);
  ...
  */
  ...
  viewDr->SetArea(-5.5*_X_SIZE, -5.5*_Y_SIZE, -0.14, 5.5*_X_SIZE, 5.5*_Y_SIZE, 0.14);
  viewDr->SetCanvas(c3);
  ...
  vMsh->SetArea(-5.5*_X_SIZE, -5.5*_Y_SIZE, -0.14, 5.5*_X_SIZE, 5.5*_Y_SIZE, 0.14);
...
//#endif
  ...
  for (int gg=0;gg<51;gg++) {
     aval2->DriftElectron((-4.8*_X_SIZE+gg*(_X_SIZE*9.6)/50),_Y_SIZE, -0.129, 0);
  }
  std::cout<<"vMsh->Plot(): ";
